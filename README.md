...
目前 GitHub 公開穩定版本為 v3.0.15：新增依 Windows 語系決定首次啟動語言，並修正沒有就緒備援線路時自動切換仍可能開啟的問題。v3.0.15 延續安裝版高權限開機自動保護、繁體中文／English 介面切換、網路自動偵測與 A/B 啟動安全：備援只會選擇 `IsReady` 線路、排除 `NotPresent` 下拉項目，沒有第二條就緒線路時會清空備援、提示使用者並關閉自動切換；非管理員不會啟動 A/B。
...
GitHub 發布步驟請看 [GitHub 發布準備](docs/GITHUB-PUBLISH.md)；v3.0.15 變更與驗證請看 [Release notes](docs/RELEASE-v3.0.15.md)。
...
.\dist\NetOptimizer-v3.0.15.exe --interface-metric-test
...
.\dist\NetOptimizer-v3.0.15.exe --support-snapshot=.\build\support.png
.\dist\NetOptimizer-v3.0.15.exe --soak-test --seconds=60
```

## 語言

啟動程式後，使用右上角的語言下拉選單切換 `繁體中文` 或 `English`。首次啟動或設定檔無法使用時，會依 Windows 語系預設：任何 `zh-*` 中文語系使用繁體中文，其他語系使用 English；已有設定與手動選擇不會被覆蓋。切換會立即套用到主畫面、系統匣選單與支持開發視窗，並保存到 `%LOCALAPPDATA%\NetOptimizer\settings.xml`。網卡名稱、測試目標與診斷資料保持原始內容。
